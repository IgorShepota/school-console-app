...
class SchoolOperationsTest {
  ...
  void findGroupsWithLessOrEqualStudentShouldWorkCorrectlyIfInputIsPositiveNumber() {
    ...
    when(groupDao.findGroupsWithLessOrEqualStudent(5)).thenReturn(mockGroups);
  ...
  void findGroupsWithLessOrEqualStudentShouldPrintMessageIfNoGroupsFound() {
    ...
    when(consoleManager.readLine()).thenReturn(userInput);
    when(consoleManager.parseInput(userInput)).thenReturn(5);
    when(groupDao.findGroupsWithLessOrEqualStudent(5)).thenReturn(mockEmptyGroups);
  ...
  void findStudentsByGroupNameShouldWorkCorrectlyIfCourseExists() {
    ...
    when(studentDao.findStudentsByCourseName(courseName)).thenReturn(enrolledStudents);

    schoolOperations.findStudentsByGroupName();

    verify(consoleManager).print("Enter the course name.");
    verify(courseDao).getCourseIdByName(courseName);
  ...
  void addNewStudentShouldWorkCorrectlyWithCorrectData() {
    ...
    verify(studentDao).save(newStudent);
  ...
  void enrollStudentToCourseShouldWorkCorrectlyIfStudentAndCourseExist() {
    ...
    when(courseDao.getCourseIdByName(newCourse.getCourseName())).thenReturn(Optional.of(newCourse));
    when(courseDao.getEnrolledCoursesForStudent(student.getId())).thenReturn(
        Collections.emptyList());
    when(courseDao.checkStudentEnrolledInCourse(student.getId(),
        newCourse.getId())).thenReturn(false);

    schoolOperations.enrollStudentToCourse();

    verify(courseDao).enrollStudentToCourse(student.getId(), newCourse.getId());
  ...
  void enrollStudentToCourseShouldNotEnrollIfCourseDoesNotExist() {
    ...
    when(studentDao.findById(validStudentId)).thenReturn(Optional.of(student));
    when(courseDao.getCourseIdByName(nonExistentCourseName)).thenReturn(Optional.empty());

    schoolOperations.enrollStudentToCourse();

    verify(courseDao, never()).enrollStudentToCourse(anyInt(), anyInt());
    verify(consoleManager).print(
  ...
  void enrollStudentToCourseShouldPrintMessageIfStudentDoesNotExist() {
    ...
    when(studentDao.findById(nonExistentStudentId)).thenReturn(Optional.empty());

    schoolOperations.enrollStudentToCourse();

    verify(consoleManager).print("Enter the student ID");
    verify(studentDao).findById(nonExistentStudentId);
  ...
  void enrollStudentToCourseShouldNotEnrollIfStudentIsAlreadyEnrolled() {
    ...
    when(consoleManager.parseInput(anyString())).thenReturn(validStudentId);
    when(studentDao.findById(validStudentId)).thenReturn(Optional.of(student));
    when(courseDao.getCourseIdByName(existingCourseName)).thenReturn(Optional.of(course));
    when(courseDao.checkStudentEnrolledInCourse(validStudentId, validCourseId)).thenReturn(true);

    schoolOperations.enrollStudentToCourse();

    verify(courseDao, never()).enrollStudentToCourse(validStudentId, validCourseId);
    verify(consoleManager).print(
  ...
  void enrollStudentToCourseShouldPrintEnrolledCoursesCorrectlyIfDataCorrect() {
    ...
    when(studentDao.findById(studentId)).thenReturn(
        Optional.of(student));
    when(courseDao.getEnrolledCoursesForStudent(studentId)).thenReturn(
        mockEnrolledCourses);

    schoolOperations.enrollStudentToCourse();

    verify(consoleManager).print("Enter the student ID");
    verify(courseDao).getEnrolledCoursesForStudent(studentId);
  ...
  void removeStudentFromCourseShouldRemoveStudentIfEnrolled() {
    ...
    when(consoleManager.parseInput(anyString())).thenReturn(validStudentId);
    when(studentDao.findById(validStudentId)).thenReturn(Optional.of(student));
    when(courseDao.getCourseIdByName(enrolledCourseName)).thenReturn(Optional.of(course));
    when(courseDao.checkStudentEnrolledInCourse(validStudentId, validCourseId)).thenReturn(true);

    schoolOperations.removeStudentFromCourse();

    verify(courseDao).removeStudentFromCourse(validStudentId, validCourseId);
  ...
  void removeStudentFromCourseShouldNotRemoveIfStudentIdIsInvalid() {
    ...
    verify(courseDao, never()).removeStudentFromCourse(anyInt(), anyInt());
    verify(consoleManager).print("Invalid input. Please enter a valid number.");
  }
  ...
  void removeStudentFromCourseShouldExitIfStudentNotEnrolled() {
    ...
    when(courseDao.checkStudentEnrolledInCourse(validStudentId, validCourseId)).thenReturn(false);
